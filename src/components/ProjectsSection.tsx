              ...
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-xs font-semibold tracking-wide transition-all ${
                  selectedCategory === cat.id
                    ...
                    : "glass-panel text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:border-cyan-500/30"
                }`}
              >
                {cat.label}
              </button>
              ...
              >
                ...
                <div className="relative h-56 w-full bg-slate-100 dark:bg-slate-950 overflow-hidden">
                  {mainImg ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-900 dark:to-slate-950">
                      <Layers className="w-12 h-12 text-slate-400 dark:text-slate-700" />
                    </div>
                  )}
                  ...
                  <div className="absolute top-4 right-4">
                    <div className="w-9 h-9 rounded-full bg-white/90 dark:bg-slate-900/80 backdrop-blur-md border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-600 dark:text-slate-300 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 group-hover:border-cyan-400 transition-colors shadow-sm">
                  ...
                  <div className="pt-2 border-t border-slate-200 dark:border-slate-800/80 space-y-3">
                    <div className="flex flex-wrap gap-1.5">
                      {project.technologies?.slice(0, 4).map((tech, idx) => (
                        <span
                          key={idx}
                          className="px-2.5 py-1 rounded-md bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-300 text-[11px] font-mono border border-slate-200 dark:border-slate-800"
                        >
                          {tech}
                        </span>
                      ))}
                      {project.technologies && project.technologies.length > 4 && (
                        <span className="px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-900 text-cyan-600 dark:text-cyan-400 text-[11px] font-mono">

...
export default function AdminLoginPage() {
  ...
  return (
    <main className="min-h-screen bg-[#060913] text-slate-100 flex items-center justify-center p-4 relative overflow-hidden">
      {/* Glow Orbs */}
      ...
      <div className="glass-panel relative z-10 w-full max-w-md p-8 rounded-3xl border border-cyan-500/30 shadow-2xl space-y-6 bg-[#090d16]/90">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          ...
          <span className="text-[10px] font-mono text-slate-500 uppercase">Turso Protected</span>
        </div>

        <div className="text-center space-y-2">
          <div className="w-14 h-14 mx-auto rounded-2xl bg-cyan-500/20 border border-cyan-500/40 flex items-center justify-center text-cyan-400 shadow-lg shadow-cyan-500/20">
            <ShieldCheck className="w-8 h-8" />
          </div>
          <h1 className="text-2xl font-bold text-white">Logowanie do Panelu Admina</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-xs font-mono text-slate-300 flex items-center gap-2">
              <KeyRound className="w-3.5 h-3.5 text-cyan-400" /> Hasło Administratora
            </label>
